...
using Images
using OffsetArrays
import ..STC: matchlsb!,readlsb,readlsbq,LSBcostfun, AbstractImageDistortion, trypmone, ind2sub
...
function suniwardcosts(cover::Array{T},sigma::T=T(1)) where {T<:AbstractFloat}
    ...
    xi = Array{Array{Float64,2},1}(undef, 3)
    R = Array{Array{Float64,2},1}(undef, 3)
    for f = 1:3
